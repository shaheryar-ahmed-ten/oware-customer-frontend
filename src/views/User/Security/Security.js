        ...
        <Grid container item xs={12} className={classes.contentContainer}>
          <Grid item xs={12} className={classes.fieldGrid}>
            ...
            <TextField
              ...
              onChange={(e) => {
                setUserFields((prevState) => ({ ...prevState, currentPassword: e.target.value }));
                setFormSuccess(null);
                setFormErrors(null);
              }}
            />
            ...
            <TextField
              ...
              onChange={(e) => {
                setUserFields((prevState) => ({ ...prevState, newPassword: e.target.value }));
                setFormSuccess(null);
                setFormErrors(null);
              }}
            />
            ...
            <TextField
              ...
              onChange={(e) => {
                setUserFields((prevState) => ({ ...prevState, confirmNewPassword: e.target.value }));
                setFormSuccess(null);
                setFormErrors(null);
              }}
            />
            {formErrors ? <Alert severity="error">{formErrors}</Alert> : null}
            {formSuccess ? <Alert severity="success">{formSuccess}</Alert> : null}
            <Button
              variant="contained"
              className={classes.saveBtn}
              onClick={() => {
                updateUserPassword(userFields);
              }}
            >
              Save
            </Button>
